...
Ne.Class(Occipital, 'Server')({
    prototype        : {
            ...
            this.app.post('*', function (req, res) {
                if (req.body.destination) {
                    if (req.body.action === "copy") {
                        occs._copyImage(req, res);
                    } else if (req.body.action === "move") {
                        occs._moveImage(req, res);
                    } else {
                        res.send(500, 'Please specify action parameter.');
                    }
                } else if (req.body.movements) {
                    occs._moveImages(req, res);
        ...
        _generateNewImage : function _generateNewImage (originalImagePath, requestedImagePath, params) {
             ...
             */
            if (params.crop) {
                params.crop = params.crop.replace(/([0-9]+)_([0-9]+)_([0-9]+)_([0-9]+)/g, '$3x$4+$1+$2');
        ...
        _moveImage : function _moveImage (req, res) {
            ...
            destinationPath = this.path.join(this.storagePath, req.body.destination);

            occs.fs.exists(sourcePath, function(exists) {
                if (exists) {
                    occs.exec('mkdir -p ' + occs.path.dirname(destinationPath) + ' && mv ' + sourcePath + ' ' + destinationPath, function (error, stdout, stderr) {
                        if (error !== null) {
                            res.send(500, 'Move Error.');
                        } else {
                            occs.exec('rmdir -p ' + occs.path.dirname(sourcePath));
                            res.send(req.body.destination);
                        }
                    });
                } else {
        ...
        _moveImages : function _moveImages (req, res) {
            var occs = this;
            var i, cont = 0;
            var movements = req.body.movements;
            var errors = [];

            var moveFile = function (source, destination) {
                source = occs.path.join(occs.storagePath, source);
                destination = occs.path.join(occs.storagePath, destination);

                occs.fs.exists(source, function(exists) {
                    if (exists) {
                        occs.exec('mkdir -p ' + occs.path.dirname(destination) + ' && mv ' + source + ' ' + destination, function (error, stdout, stderr) {
                            if (error !== null) {
                                errors.push('Error when moving ' + source + ' to ' + destination);
                            } else {
                                occs.exec('rmdir -p ' + occs.path.dirname(source));
                            }
                            ...
                            if (cont >= movements.length) {
                                if (errors.length === 0) {
                                    ...
                                    res.send(500, 'There were errors during the process: \n' + errors.join('\n'));
                                }
                            }
                        });
                    } else {
                        errors.push('Source ' + occs.path.relative(occs.storagePath, source) + ' does not exist.');
                        cont += 1;
                        if (cont >= movements.length) {
                            if (errors.length === 0) {
                                res.send(200, movements);
            ...
            for (i = 0; i < movements.length; i+=1) {
                moveFile(movements[i].source, movements[i].destination);
            };
        },

        _copyImage : function _copyImage (req, res) {
            var occs = this;
            var sourcePath, destinationPath;
            ...
            occs.fs.lstat(sourcePath, function(error, stats) {
                var cmd;
                if (error === null) {
                    if (stats.isDirectory() === true) {
                        cmd = 'mkdir -p ' + occs.path.dirname(destinationPath) + ' && cp -r ' + sourcePath + ' ' + destinationPath;
                    } else {
                        cmd = 'mkdir -p ' + occs.path.dirname(destinationPath) + ' && cp ' + sourcePath + ' ' + destinationPath;
                    }

                    occs.exec(cmd, function (error, stdout, stderr) {
                        if (error !== null) {
                            res.send(500, 'Copy Error.');
                        } else {
                            res.send(req.body.destination);
                        }
                    });
        ...
        _uploadImage : function _uploadImage (req, res) {
            ...
            if (req.url.match(/\/[^\/]+\.[^\/]+$/)) {
                ...
                fileName = req.files.file.name.replace(/\.(?!\w+$)/g,'_').replace(/[^a-zA-Z0-9\._]/g,'_');
            }
            ...
            occs.exec('mkdir -p ' + occs.path.join(occs.storagePath, destinationFolder) + ' && mv ' + req.files.file.path + ' ' + newFilePath, function (error, stdout, stderr) {
                if (error !== null) {
                    debugger;
                    res.send(500, 'Upload Error ' + req.url);
                } else {
                    ...
                    // Get new Image data.
                    occs.lobe.getImageInfo(newFilePath, {
                        onSuccess : function (info) {
                            info.path = occs.path.join('/', occs.path.dirname(req.url), fileName);
                            res.json(info);
                        }
                    });
                }
        ...
        _uploadImageBase64 : function _uploadImageBase64 (req, res) {
            ...
            if (req.url.match(/\/[^\/]+\.[^\/]+$/)) {
                ...
                occs.exec('mkdir -p ' + occs.path.join(occs.storagePath, destinationFolder), function (error, stdout, stderr) {
                    if (error !== null) {
                        ...
                        occs.fs.writeFile(newFilePath, req.body.file64, 'base64', function (ferr) {
                            if (ferr !== null) {
                                res.send(500, 'Upload Error: Couldn\'t write to file. ' + req.url);
                            } else {
                                // Resize image to its maximum
                                occs.lobe.processSync(newFilePath, newFilePath, {
                                    outputOptions : [
                ...
                res.send(500, 'Please specify the file name. ' + req.url);
            }
        },
